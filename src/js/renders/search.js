...
// Filtreler - Filters
async function generateAreaFiltersMarkup() {
  try {
...
function createAreaMarkupFilters(ivent) { // Alan filtrelerinin oluşturma - Area filters creation
  const { name } = ivent;
  return `<option value="${name}">${name}</option>`;
}

function createIngredMarkupFilters(ivent) { // Malzemeler filtrelerinin oluşturma - Ingredients filters creation
...
async function createAreaFilters() {
  try {
    const areasMarkup = await generateAreaFiltersMarkup();
